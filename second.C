...
class LPetProcessor : public base::EventProc {
	...
	//TH2F* h_Layer_TotVsStraw[LAYERS];
       ...
       //     fstream infile("/home/pandastraws/go4/new_release/trb3/sts2test/sts2_map.txt");
        ...
        h_driftTimeVsTOT = new TH2F("DriftTime_vs_TOT", "DriftTime_vs_TOT;Drift Time;Time Over Threshold", 500, 0, 500, CHANNELS, 0, CHANNELS);
		...
		//cout<<"Layer :"<<loc.layer<<"\t"<<trailTimesNum[k] <<"\t"<< leadTimesNum[k]<<"\t"<<leadTimes[k][l] <<"\t"<< trailTimes[k][l]<<"\t"<<Tot[k][TotNum[k]]<<endl;
			...
			//cout<<"Layer : "<<loc.layer<<"\t"<<loc.straw<<"\t"<<str_in_mod<<endl;
				...
				//printf("%lf\n",drifttime);
                              ...
                              //  if ( (drifttime >= 0 and drifttime <= 500) and (Tot[k][TotNum[k]] >= 0 and Tot[k][TotNum[k]] <= 600))
                                //    {
                                        h_Tot->Fill(Tot[k][TotNum[k]]); //General
                                      //  h_driftTime->Fill(0);
                                      //  h_driftTimeVsTOT->Fill(drifttime,Tot[k][TotNum[k]]);
					...
					(module%8==0) ? h_strawMult->Fill(((loc.layer -1) *256) + loc.straw) - 96 : h_strawMult->Fill(((loc.layer -1) *256) + loc.straw);
...
stef1_mult[ly]=0;
		...
		for (int ht=0; ht<CHANNELS; ht++){
			if (TotNum[ht]>0) {hit_mult++;}	
		}
		for (int md=0; md<MODULES; md++){
			if (TotMod[md]>0) {mod_mult++;}		
		}
		for (int pl=0; pl<LAYERS*2; pl++){
			if (TotPlane[pl]>0) {plane_mult++;}
		...
		for (int ly=0; ly<LAYERS; ly++){
			if (TotLay[ly]>0) {layer_mult++;}
			for(int j=0; j<STRAWS; j++){
				if(TotStraw[ly][j]>0) stef1_mult[ly]++;
			}		
		}

		if(s2_t == true){
			for (int ly=0; ly<4; ly++){
				h_sts2_eff_Lstraws->Fill(ly+1,stef1_mult[ly]);
			}
		}
		if(s1_t == true){
		...
		if(s2_t == true)h_sts_entries->Fill(2);
